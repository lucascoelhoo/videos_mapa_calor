...
for indice_aux in range(len(datas)):
    for localidade,num in zip(localidades['regiao'],range(len(localidades['regiao']))): 
        if (localidade in patch_regions_array and localidade in lista_dataframes[indice_aux]['regiao']):
            valor=int(lista_dataframes[indice_aux].at[localidade,'obitos'])
            if(valor>max_value):
                max_value=valor
print(max_value)     
...
def animate(i):
    ...
    for localidade,num in zip(localidades['regiao'],range(len(localidades['regiao']))): 
        if (localidade in patch_regions_array and localidade in lista_dataframes[indice]['regiao']):
            #print(num)
            valor=int(lista_dataframes[indice].at[localidade,'obitos'])
            #print(valor)
...
Writer = animation.FFMpegWriter(fps=5, metadata=dict(name='Espalhamento da doença COVID-19 no Distrito Federal (Número de Óbitos)',artist='Lucas Coelho de Almeida',year='2020'
