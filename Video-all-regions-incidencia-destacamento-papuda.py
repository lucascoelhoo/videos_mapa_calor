...
def animate(i):
    ...
    for localidade,num in zip(localidades['regiao'],range(len(localidades['regiao']))): 
        if (localidade in patch_regions_array and localidade in lista_dataframes[indice]['regiao']):
            #print(num)
            valor=float(lista_dataframes[indice].at[localidade,'incidencia'])
            #print(valor)
            ...
            if(not(localidade=="populacaoprivadadeliberdade") and valor>greatest):
                greatest=valor
...
anim.save('All-Regions-incidencia-destacamento-papuda.mp4', writer=Writer )
print(greatest)
